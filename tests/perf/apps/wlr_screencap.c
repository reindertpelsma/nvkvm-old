...
{
	/* Render-target bo: let the driver pick a render-capable modifier
	 * (NVIDIA's LINEAR is texture-only; block-linear supports render). */
	cap_bo = gbm_bo_create(gbm, cap_w, cap_h, cap_fmt,
...
int main(int argc, char **argv)
{
	...
	for (int i = 2; i < argc - 1; i++)
		if (!strcmp(argv[i], "--ppm")) ppm = argv[i + 1];
	...
	for (int i = 0; i < nframes; i++) {
		...
		if (frame_ready) ok++;
		else { fprintf(stderr, "frame %d failed\n", i); break; }
	}
	double dt = now_s() - t0;
