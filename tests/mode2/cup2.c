/* Mode-2 post-cuInit probe: every call wrapped + flushed to pinpoint the wall. */
#include <cuda.h>
#include <stdio.h>
...
int main(void){
    ...
    size_t tot=0; CK(cuDeviceTotalMem(&tot,d)); printf("totalMem=%zu MiB\n",tot>>20); fflush(stdout);
    CUcontext ctx; CK(cuCtxCreate(&ctx,0,d));
    printf("CTX OK\n"); fflush(stdout);
    CUdeviceptr dp; CK(cuMemAlloc(&dp,4096));
    printf("MEMALLOC OK 0x%llx\n",(unsigned long long)dp); fflush(stdout);
