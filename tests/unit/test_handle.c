...
/* ── Tests: /dev/nvidiactl dedupe ───────────────────────────────────────────── */

/* Helper: stand up a "ctl" handle the same way the live path does
 * (alloc_pending + attach_fd) using a memfd in place of /dev/nvidiactl. */
static uint32_t open_test_ctl_handle(struct nvkvm_handle_table *t,
                                     uint32_t session_id)
{
    uint32_t id = 0;
    int ret = nvkvm_handle_alloc_pending(t, session_id, 0 /* NVKVM_DEV_CTL */,
                                         &id);
    if (ret < 0)
        return 0;
    int fd = dup(stub_dev_fd);
    if (fd < 0) {
        nvkvm_handle_abort_open(t, id);
        return 0;
    }
    ret = nvkvm_handle_attach_fd(t, id, fd);
    if (ret < 0) {
        close(fd);
        nvkvm_handle_abort_open(t, id);
        return 0;
...
TEST(test_dedupe_ctl_returns_same_handle_in_session)
{
    struct nvkvm_handle_table t;
    nvkvm_handle_table_init(&t);

    uint32_t id1 = open_test_ctl_handle(&t, 1);
    ASSERT_NE(id1, 0);

    uint32_t id2 = nvkvm_handle_dedupe_ctl(&t, 1);
    ASSERT_EQ(id1, id2);

    struct nvkvm_handle *h = nvkvm_handle_get(&t, id1);
    ASSERT_NOTNULL(h);
    ASSERT_EQ(h->guest_refcount, 2u);

    nvkvm_handle_table_fini(&t);
}

TEST(test_dedupe_ctl_different_session_no_match)
{
    struct nvkvm_handle_table t;
    nvkvm_handle_table_init(&t);
    ...
    /* dedupe lookup for a different session should NOT match */
    uint32_t got = nvkvm_handle_dedupe_ctl(&t, 2);
    ASSERT_EQ(got, 0u);

    nvkvm_handle_table_fini(&t);
}

TEST(test_dedupe_close_decrements_then_actually_closes)
{
    struct nvkvm_handle_table t;
    nvkvm_handle_table_init(&t);
    ...
    /* Dedupe twice — refcount goes to 3 */
    ASSERT_EQ(nvkvm_handle_dedupe_ctl(&t, 1), id1);
    ASSERT_EQ(nvkvm_handle_dedupe_ctl(&t, 1), id1);
    struct nvkvm_handle *h = nvkvm_handle_get(&t, id1);
    ASSERT_EQ(h->guest_refcount, 3u);

    /* First close: refcount 3->2, slot still in_use, fd still open */
    ASSERT_EQ(nvkvm_handle_close(&t, id1), 0);
    h = nvkvm_handle_get(&t, id1);
    ASSERT_NOTNULL(h);
    ASSERT_EQ(h->guest_refcount, 2u);
    ASSERT_NE(h->fd, -1);

    /* Second close: refcount 2->1, still alive */
    ASSERT_EQ(nvkvm_handle_close(&t, id1), 0);
    h = nvkvm_handle_get(&t, id1);
    ASSERT_NOTNULL(h);
    ASSERT_EQ(h->guest_refcount, 1u);

    /* Third close: refcount 1->0, actual close happens */
    ASSERT_EQ(nvkvm_handle_close(&t, id1), 0);
    ASSERT_NULL(nvkvm_handle_get(&t, id1));
