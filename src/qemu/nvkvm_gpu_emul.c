...
#define NVKVM_MAX_BAR1PG 64
    ...
    uint32_t m2_gr_tsg_sched;   /* #12 cont.34: which GR TSG handle we last GPFIFO_SCHEDULE'd.
                                 * doorbell_setup schedules the GR TSG once, but early-returns on
                                 * m2_doorbell_ready (sticky), so a 2nd context's NEW GR TSG was
                                 * never scheduled -> its 8 rl=0 GR channels rang but the host
                                 * never consumed (gp_get stuck 0) -> 8/16 pool semas never
                                 * advanced -> #12 residual. The ring loop schedules the GR TSG
    ...
    if (off == NVKVM_VF_DOORBELL) {
        ...
        for (int i = 0; i < s->chan_n; i++) {
            ...
            if (s->m2_usermode_qva && c->token_valid && c->client != 0xc1d00001u) {
                ...
                if (c->tsg && !c->scheduled && c->tsg != s->m2_gr_tsg_sched) {
                    uint8_t sp[3]; memset(sp, 0, sizeof(sp)); sp[0] = 1;
                    uint32_t sst = 0xffff;
                    int src = nvkvm_m2_control1(s, c->client, c->tsg, 0xa06c0101u,
                                                sp, sizeof(sp), &sst);
...
static void nvkvm_m2_doorbell_setup(NvkvmGpuEmul *s, uint32_t client)
{
    ...
    if (s->m2_gr_tsg) {
        ...
        if (src == 0 && sst == 0) { s->m2_gr_tsg_sched = s->m2_gr_tsg; }  /* #12 cont.34 */
...
static uint64_t nvkvm_m2_resolve_fb_inner(NvkvmGpuEmul *s, uint64_t va)
{
    for (int v = 0; v < s->chan_vas_n; v++) {
        bool sy = false;
...
{
    uint64_t t0 = nvkvm_now_ns();
    uint64_t r = nvkvm_m2_resolve_fb_inner(s, va);
...
static bool nvkvm_m2_pdb_is_compute(NvkvmGpuEmul *s, uint64_t pdb)
{
    ...
    for (int i = 0; i < s->chan_vas_n; i++) {
        ...
        if (cl && (cl == s->m2_gr_client || nvkvm_m2_is_user_ce(s, cl))) { return true; }
    }
    return false;
}
...
static void nvkvm_m2_exec_doorbell(NvkvmGpuEmul *s)
{
    ...
    for (int i = 0; i < s->chan_n; i++) {
         ...
         * (tracked by m2_gr_tsg_sched); the M5.33 st=0x57 noise the old skip avoided was
         * RE-scheduling an already-scheduled TSG, which this one-shot guard prevents. */
        if (is_gr && s->m2_gr_tsg && s->m2_gr_tsg != s->m2_gr_tsg_sched) {
            uint8_t sp[3]; memset(sp, 0, sizeof(sp)); sp[0] = 1;
            uint32_t sst = 0xffff;
            int src = nvkvm_m2_control1(s, c->client, s->m2_gr_tsg, 0xa06c0101u,
                                        sp, sizeof(sp), &sst);
            if (src == 0 && sst == 0) { s->m2_gr_tsg_sched = s->m2_gr_tsg; }
