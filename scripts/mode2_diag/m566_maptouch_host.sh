#!/usr/bin/env bash
# m566_maptouch_host.sh — ON HOST. Map-on-touch CE-forward verification (supersedes m565's
# window probe). Boots with NVKVM_M2CEFWD=1 and the map-on-touch code, runs cup7@64MiB, and
...
# engaged, off-BAR1 dst), gpga FAILED = 0 (no BAR1 wall), no Xid, no hang, cup7 VERDICT
...
( for t in $(seq 1 65); do echo "t$((t*2))s: $(nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader 2>/dev/null)"; sleep 2; done ) > /tmp/m566_util.log 2>&1 &

echo "==> run cup7 (foreground, timeout 130s, ${MB} MiB) — rc=124 => HANG (BAR1 wall)"
$SSHG "CUP7_MB=${MB} CUP7_TIMEOUT=120 bash /tmp/cup7_run_guest.sh" > /tmp/m566_guest.log 2>&1
# NOTE: this RC is ssh's, NOT cup7's; trust the inline 'exit rc=NNN' in the cup7 stdout below.
echo "--- cup7 stdout ---"
grep -iE "CUP7|VERDICT|RESULT|FIRST bad|FAIL|exit rc|CTX OK|SYNC OK|DONE" /tmp/m566_guest.log | head -30

QE=$(wc -l < "$QLOG" 2>/dev/null || echo 0)
sed -n "$((QB+1)),${QE}p" "$QLOG" > /tmp/m566_delta.txt
D=/tmp/m566_delta.txt
echo ""; echo "============ M566 MAP-ON-TOUCH EVIDENCE ============"
echo "--- cup7 VERDICT (PASS=D1 byte-exact = THE WIN) ---"
grep -E "CUP7 VERDICT|CUP7 RESULT|FIRST bad|exit rc" /tmp/m566_guest.log
...
grep -oE "[0-9]+ %" /tmp/m566_util.log | sort -rn | head -1
grep -oE "[0-9]+ MiB" /tmp/m566_util.log | sort -rn | head -1
