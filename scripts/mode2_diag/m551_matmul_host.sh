#!/usr/bin/env bash
# m551_matmul_host.sh — ON HOST. North-star step 1: GR KERNEL LAUNCH (cup3).
# DEFAULT mode (NOT m2hostsem). Fresh boot, run cup3 (cuLaunchKernel out=in*3+1) to
...
echo "==> stage cup3 + runner"
scp -q -P $PORT -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
  /workspace/nvkvm/tests/mode2/cup3.c /workspace/nvkvm/scripts/mode2_diag/cup3_run_guest.sh \
  ubuntu@localhost:/tmp/ 2>/dev/null
QB=$(wc -l < "$QLOG" 2>/dev/null || echo 0)

echo "==> host GPU util sampler (background, 90s)"
( for t in $(seq 1 45); do echo "util_t$((t*2))s: $(nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader 2>/dev/null)"; sleep 2; done ) > /tmp/m551_util.log 2>&1 &

echo "==> run cup3 to completion (foreground, timeout 120s)"
$SSHG 'CUP3_TIMEOUT=120 bash /tmp/cup3_run_guest.sh' > /tmp/m551_guest.log 2>&1
echo "--- cup3 stdout ---"
grep -iE "ptxjit|CTX|MODULE|FUNC|MEMALLOC|LAUNCH|SYNC|KERNEL rv|PASS|MISMATCH|FAIL|exit rc" /tmp/m551_guest.log | head -40

QE=$(wc -l < "$QLOG" 2>/dev/null || echo 0)
sed -n "$((QB+1)),${QE}p" "$QLOG" > /tmp/m551_delta.txt
D=/tmp/m551_delta.txt
echo ""; echo "============ M551 KERNEL-LAUNCH EVIDENCE ============"
echo "--- util peak ---"; grep -oE "[0-9]+ %" /tmp/m551_util.log | sort -rn | head -3; tail -2 /tmp/m551_util.log
