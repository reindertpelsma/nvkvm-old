#!/usr/bin/env bash
# cup4_run_guest.sh — ON THE GUEST. Same module/firmware/node prep as cup2_run_guest.sh,
# but builds + runs cup4 (GR KERNEL LAUNCH proof) to completion (bounded by timeout).
...
gcc -O0 -g -o /tmp/cup4 /tmp/cup4.c -lcuda 2>&1 | tail -3

echo "=== cup4 (foreground, timeout ${TIMEOUT}s) ==="
LD_LIBRARY_PATH=$GUESTLIB timeout --signal=INT "$TIMEOUT" stdbuf -oL -eL /tmp/cup4
RC=$?
echo "=== cup4 exit rc=$RC (124=timeout/hang) ==="
