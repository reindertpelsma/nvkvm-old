...
for i in 1 2 3; do
  LD_LIBRARY_PATH="/usr/local/nvidia-guest/lib:/lib/x86_64-linux-gnu" \
    LD_PRELOAD=/tmp/nvmemsnap.so NVSNAP=/tmp/snap_guest_$i.txt NVSNAP_CLASS=0xc7c0 NVSNAP_MAX=65536 \
    /tmp/cup2 >/tmp/cup2_$i.log 2>&1
  echo "guest run $i: exit=$? post=$(grep -c '^SNAP post' /tmp/snap_guest_$i.txt 2>/dev/null) regs=$(grep -c '^REG' /tmp/snap_guest_$i.txt 2>/dev/null) last=$(tail -1 /tmp/cup2_$i.log)"
done
